...
class WarpMapMlx(WarpMapBase):
    ...
    def chain(self, target):
        """Chain displacement maps for MLX arrays.

        MLX arrays do not expose `.copy()` with NumPy/CuPy semantics,
        so we build a new array via addition.
        """
    ...
    def median_filter(self):
        ...
        filtered = median_filter_mlx(self.warp_field, size=[1, 3, 3, 3], mode="nearest")
    ...
    def push_coordinates(self, coords, negative_shifts=False):
        ...
        warp_field_np = np.array(warp_field)
        coords_np = np.array(coords_blocked)

        for idim in range(3):
            shift_vals = map_coordinates(warp_field_np[idim], coords_np, order=1, mode="nearest")
            shifts[idim] = mx.array(shift_vals, dtype=mx.float32)
    ...
    def to_h5(self, h5_path, group="warp_map", compression="gzip", overwrite=True):
        ...
        with h5py.File(h5_path, "a") as f:
            ...
            grp.create_dataset("warp_field", data=np.array(self.warp_field), compression=compression)
            grp.create_dataset("block_size", data=np.array(self.block_size))
            grp.create_dataset("block_stride", data=np.array(self.block_stride))
    ...
    def from_h5(cls, h5_path, group="warp_map", backend="auto"):
        ...
        Returns:
            WarpMapMlx: The loaded WarpMap object.
        """
        import h5py
...
class RegistrationPyramidMlx:
    ...
    Args:
        ...
        recipe: Settings for each level of the pyramid.
...
class ProjectorMlx(BaseModel):
    ...
    def __call__(self, vol_blocks, axis):
        ...
        if self.dog:
            out = dogfilter_mlx(out, [0, 0, 0, *low], [0, 0, 0, *high])
...
class RegFilterMlx(BaseModel):
    ...
    def __call__(self, vol, reg_mask=None):
        ...
        if np.any(np.array(self.soft_edge) > 0):
            vol = soften_edges_mlx(vol, self.soft_edge)
        ...
        if self.dog:
            vol = dogfilter_mlx(vol, self.low, self.high)
